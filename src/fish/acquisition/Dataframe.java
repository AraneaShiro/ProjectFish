...
public abstract class Dataframe implements Statistique {
    ...
    public double calculerCorrelation(int col1, int col2) throws OutOfBoundException {
        ...
        double moy1 = valeurs1.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        double moy2 = valeurs2.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);

        double numerateur = 0.0;
        double denomCarre1 = 0.0;
        double denomCarre2 = 0.0;

        for (int i = 0; i < taille; i++) {
            double ecart1 = valeurs1.get(i) - moy1;
            double ecart2 = valeurs2.get(i) - moy2;
            numerateur += ecart1 * ecart2;
            denomCarre1 += Math.pow(ecart1, 2);
            denomCarre2 += Math.pow(ecart2, 2);
        }

        double denominateur = Math.sqrt(denomCarre1 * denomCarre2);
        return denominateur == 0.0 ? 0.0 : numerateur / denominateur;
    }
}